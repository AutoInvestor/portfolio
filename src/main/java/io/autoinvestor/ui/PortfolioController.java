...
public class PortfolioController {
    ...
    public ResponseEntity<Void> addHolding(
        ...
        for (HoldingRequestDTO holding : holdingRequestDTO) {
            newHoldingCommandHandler.handle(
                    new NewHoldingCommand(
                            holding.assetId(), userId, holding.amount(), holding.boughtPrice()));
        }
    ...
    public ResponseEntity<List<GetHoldingResponse>> getHoldings(
            @RequestHeader(value = "X-User-Id", required = true) String userId) {
        List<HoldingsReadModelDTO> documents =
                getHoldingsQueryHandler.handle(new GetHoldingsQuery(userId));
    ...
    public ResponseEntity<Void> putHolding(
            ...
            @RequestBody List<HoldingRequestDTO> holdingRequestDTO) {
        for (HoldingRequestDTO holding : holdingRequestDTO) {
            updateHoldingCommandHandler.handle(
                    new UpdateHoldingCommand(
                            userId, holding.assetId(), holding.amount(), holding.boughtPrice()));
        }
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
